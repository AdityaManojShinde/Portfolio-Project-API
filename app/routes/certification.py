...
def get_user_certifications(db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
    """Get all certifications for current user."""
    return CertificationService.get_user_certifications(db, current_user.id)


@router.get("/{certification_id}", response_model=Certification)
def get_certification(certification_id: str, db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
    """Get a specific certification."""
    certification = CertificationService.get_certification(db, certification_id)
    # Verify ownership
    if certification.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to access this certification")
...
def update_certification(
    ...
    return CertificationService.update_certification(db, certification_id, certification_update)


@router.delete("/{certification_id}")
def delete_certification(certification_id: str, db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
