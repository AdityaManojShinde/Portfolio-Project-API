...
def get_user_education(db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
    """Get all education entries for current user."""
    return EducationService.get_user_education(db, current_user.id)


@router.get("/{education_id}", response_model=Education)
def get_education(education_id: str, db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
    ...
    if education.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to access this education record")
    return education
...
def delete_education(education_id: str, db: DatabaseSession, current_user: Annotated[User, Depends(get_current_user)]):
