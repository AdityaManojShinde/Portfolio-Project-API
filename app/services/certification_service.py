...
class CertificationService:
    ...
    @staticmethod
    def create_certification(db: Session, user_id: str, certification_data: CertificationCreate) -> CertificationDB:
        """Create a new certification entry."""
        try:
            certification_dict = certification_data.model_dump(exclude_unset=True)
            certification_dict = CertificationService._convert_urls_to_strings(certification_dict)
    ...
    def get_certification(db: Session, certification_id: str) -> CertificationDB:
        ...
        certification = db.query(CertificationDB).filter(CertificationDB.id == certification_id).first()
    ...
    def get_user_certifications(db: Session, user_id: str) -> list[CertificationDB]:
        ...
        return db.query(CertificationDB).filter(CertificationDB.user_id == user_id).all()

    @staticmethod
    def update_certification(db: Session, certification_id: str, update_data: CertificationUpdate) -> CertificationDB:
